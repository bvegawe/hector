...
subroutine run_brick(ns, tstep, &
                     temp_forcing_in, delta_ocheat_in, &                             ! proxy DOECLIM input/ouput
                     ...
                     tee_c, tee_a, tee_rho, tee_sa, luse_tee, &                ! TEE input/output
...
!   delta_ocheat_in     ocean heat change [J]
! 
! Parameters:
!   tstep            	time step [y]
...
!   tee_c               heat capacity of conservative temperature [J/kg/K]
!   tee_a               ocean-averaged thermal expansion coefficient [kg/m3/K]
!   tee_rho             ocean-average density [kg/m3]
!   tee_sa              total ocean surface area [m2]
!   luse_tee            whether to use explicit TE SLR calculation [1 or 0]
...
!===============================================================================
    ...
    real(DP), dimension(ns), intent(IN) :: delta_ocheat_in
...
! parameters
    ...
    real(DP),     intent(IN) :: tee_c
    real(DP),     intent(IN) :: tee_a
    real(DP),     intent(IN) :: tee_rho
    real(DP),     intent(IN) :: tee_sa
